...
def main():
    ...
    # Create LTE Users
    LTE_users_list = service.createLTEUsers(scene_params)
    print("\nLTE Users:")
    for user in LTE_users_list:
        print(f"ID: {user.ueID}, Location: ({user.x}, {user.y})")

    # Create WiFi Users
    wifi_users_list = service.createWifiUsers(scene_params)
    print("\nWiFi Users:")
    ...
    for tf in tqdm(range(0,scene_params.times_frames)):
        ...
        print(rl.currentPfactor)
        print("New State: ",rl.current_state)
        rl.original_power = [0.19,0.19,0.19]
        lte_bs_list[0].format = generate_format(rl.current_state)
        if rl.current_action == (0,0,0,1,0,0) or rl.current_action == (0,0,0,-1,0,0)or rl.current_action == (0,0,0,0,1,0)or rl.current_action == (0,0,0,0,-1,0)or rl.current_action == (0,0,0,0,0,1)or rl.current_action == (0,0,0,0,0,-1):
            if rl.current_state[3] == 0:
                scene_params.pTxLTE[0] = 0.19
            if rl.current_state[4] == 0:
                scene_params.pTxLTE[1] = 0.19
            if rl.current_state[5] == 0:
                scene_params.pTxLTE[2] = 0.19
            if rl.current_state[3] == 1:
                scene_params.pTxLTE[0] = 0.19/1.05
            if rl.current_state[4] == 1:
                scene_params.pTxLTE[1] = 0.19/1.05
            if rl.current_state[5] == 1:
                scene_params.pTxLTE[2] = 0.19/1.05
            if rl.current_state[3] == 2:
                scene_params.pTxLTE[0] = 0.19/1.1
            if rl.current_state[4] == 2:
                scene_params.pTxLTE[1] = 0.19/1.1
            if rl.current_state[5] == 2:
                scene_params.pTxLTE[2] = 0.19/1.1
            ...
            print("New Power: ",scene_params.pTxLTE)
        ...
        for subframe_iterator in range(10):
            ...
            if subframe_iterator == 9:
                ...
                if scene_params.vary_load == 1 and vary_for_every <=0 :
                    ...
                    LTE_users_list = service.createLTEUsers(varyparams)
                    wifi_users_list = service.createWifiUsers(varyparams)
    ...
    #lte_bs_list[0].format = format[rl.initial_state]
    for tf in tqdm(range(0,scene_params.test_eps)):
        ...
        lte_bs_list[0].format = generate_format(rl.current_state)
        if rl.current_action == (0,0,0,1,0,0) or rl.current_action == (0,0,0,-1,0,0)or rl.current_action == (0,0,0,0,1,0)or rl.current_action == (0,0,0,0,-1,0)or rl.current_action == (0,0,0,0,0,1)or rl.current_action == (0,0,0,0,0,-1):
            if rl.current_action[3] == 1 or rl.current_action[3] == -1:
                scene_params.pTxLTE[0] = rl.originalPower[0] / rl.currentPfactor
            elif rl.current_action[4] == 1 or rl.current_action[4] == -1:
                scene_params.pTxLTE[1] = rl.originalPower[1] / rl.currentPfactor
            elif rl.current_action[5] == 1 or rl.current_action[5] == -1:
                scene_params.pTxLTE[2] = rl.originalPower[2] / rl.currentPfactor
        ...
        for subframe_iterator in range(10):
            ...
            if subframe_iterator == 9:
                ...
                if scene_params.vary_load == 1 and vary_for_every <=0 :
                    ...
                    vary_for_every = scene_params.vary_for_every
                    #End of subframe loop
        print(LTECountS, WifiCountS, total_PRBs, total_Wifi_slots)
        for i in range(3):
            U_LTE = (LTECountS[i])/total_PRBs[i] if total_PRBs[i] != 0 else 0
        U_Wifi = (WifiCountS)/total_Wifi_slots if total_Wifi_slots != 0 else 0
